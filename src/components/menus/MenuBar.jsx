...
const MenuBar = () => {
    const userData = useContext(UserProvider.context);     //first getting data from context
    const loginType = !_.isEmpty(userData) ? _.find(data, d => d.name === userData.provider) : {};     // now figuring ot is user logged in or not   if user data obj is not empty then find the data key that matches 
